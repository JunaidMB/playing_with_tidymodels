...
fields <- sort(rep.int(c(1:5), 5))
quads <- rep(c(1:5), 5)
values <- rnorm(n = 25, mean = 3)
crop <- sample(x = c("linseed", "barley", "olive", "potato", "cauliflower"), size = 25, replace = TRUE, prob = c(0.5, 0.2, 0.05, 0.20, 0.05))
red_density_state <- sample(x = c(1,2,3), size = 25, replace = TRUE, prob = c(0.5, 0.4, 0.1))
junk_col1 <- rnorm(n = 25, mean = 50)
junk_col2 <- rnorm(n = 25, mean = 40)
junk_col3 <- rnorm(n = 25, mean = 30)
...
## Split the Training set further into a validation set
set.seed(234)
df_val <- validation_split(df_train, prop = 0.8)

# Create Recipe ----
## Columns to remove
cols_to_rmv <- c("junk_col1", "junk_col2", "junk_col3")
...
set.seed(10)
rf_grid <- grid_random(mtry() %>% finalize(df %>% select(values, crop)), # Predictor Values to be used from Raw data
                       trees(), 
                       size = 20) %>% 
  arrange(mtry, trees)

## Tune candidate models in the grid
## Model is built on the training data and validated on the validation set
set.seed(345)
...
## Extract predictions from the best model for the validation set
rf_best_model_predictions <- rf_tuned %>% 
  collect_predictions(parameters = rf_best_model) %>% 
  mutate(model = "Random Forest")

## Compute confusion matrix on the validation set
rf_best_model_predictions %>% 
...
## Pull out permutation importance from workflow
# final_rf$fit$fit$fit$importance
# importance(final_rf$fit$fit$fit, type = 1)
...
rand_forest(trees = rf_best_model$trees, mtry = rf_best_model$mtry) %>% # Best parameters from the finalised workflow
  ...
  fit(red_density_state ~ .,
      data = df_train_bake %>% 
        select(-fields, -quads, -.row) # Remove ID columns
      ) %>% 
